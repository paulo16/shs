...
TabularTablesPaiements.Paiements = new Tabular.Table({
  ...
  columns: [
    { data: "agent.lastName", title: "Nom agent" },
    { data: "agent.numero_agent", title: "No agent" },
    { data: "client.ref_contrat", title: "ref contrat" },
    { data: "client.nom", title: "Nom" },
    { data: "client.cin", title: "Cin" },
    { data: "client.province", title: "Province" },
    { data: "client.commune", title: "commune" },
    { data: "client.village", title: "village" },
    {
    ...
    {
      title: "Actions",
      tmpl: Meteor.isClient && Template.PaiementsActionBtns, class: "col-md-2"
    },
  ],
    ...
    selector() {
      ...
      if (filtre != '' && filtre != undefined) {
        ...
        if (filtre.date_mise_service != '' && filtre.date_mise_service != undefined) {
          ...
          query.$and.push({ "client.date_mise_service": { '$gte': date_mise_service_debut, '$lt': date_mise_service_fin } });
        }

        if (filtre.cin != '' && filtre.cin != undefined) {
          query.$and.push({ 'client.cin': { '$regex': filtre.cin, '$options': 'i' } });
        }

        if (filtre.province != '' && filtre.province != undefined) {
          query.$and.push({ 'client.province': { '$regex': filtre.province, '$options': 'i' } });
        }

        if (filtre.dateDebut != '' && filtre.dateDebut != undefined && filtre.dateFin != '' && filtre.dateFin != undefined) {
          query.$and.push({ date_paiement_manuelle: { '$gt': filtre.dateDebut, '$lt': filtre.dateFin } });
        }
      ...
      return { 'client.province': Session.get('FirstProvince') };
    },
  });
